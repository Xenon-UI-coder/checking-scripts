--AOC
--[[
loadstring Version is      loadstring(game:HttpGet('https://raw.githubusercontent.com/AOCScript/AOC/main/Da-hood'))()      (Open Source)
]]
...
do
	Library = {
		Open = true;
		Accent = Color3.fromRGB(12, 0, 228);
		Pages = {};
		Sections = {};
		Flags = {};
		UnNamedFlags = 0;
	...
	do
		...
		function Library:New(Properties)
			...
			Logo.Name = "Logo"
			Logo.Image = "http://www.roblox.com/asset/?id=83285511653957" --AOC logo [AOC] (Beta)
			...
			local Logo = Instance.new("ImageLabel")
			Logo.Name = "Logo"
			Logo.Image = "http://www.roblox.com/asset/?id=0" -- [137300464252641]--AOC logo [A]
			Logo.ScaleType = Enum.ScaleType.Fit
			Logo.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			Logo.BackgroundTransparency = 1
			Logo.BorderColor3 = Color3.fromRGB(0, 0, 0)
		...
		function Library:Watermark(Properties)
			local Watermark = {
				Name = (Properties.Name or Properties.name or "AOC | placeholder");
				AnimateText = nil;
			}
			--
			local Outline = Instance.new("Frame")
			...
			task.spawn(function()
				while task.wait() do
					for i = 1, #"AOC" do
						Watermark.AnimateText = string.sub("AOC", 1, i) .. "";
...
--// window locals
local default_color = Color3.fromRGB(12, 0, 228);
...
screen_gui.Name = "https://discord.com/invite/QwvGH6SMz6";
...
screen_gui_2.Name = "https://discord.com/invite/QwvGH6SMz6";
...
do
	...
	do
		...
		instances["local_text"] = utility.instance_new("TextLabel", { 
			Name = "https://discord.com/invite/QwvGH6SMz6",
			Parent = screen_gui_2,
			Text = '<font color="rgb(12, 0, 228)">AOC</font><font color="rgb(255, 255, 255)">OnTop</font>',
    ...
    do
        ...
        instances["target_ui"]["logo"] = utility.instance_new("ImageLabel", {
            Name = "Logo";
            Parent = instances["target_ui"]["info"];
            BackgroundColor3 = Color3.fromRGB(12, 0, 228);
            BackgroundTransparency = 1.0;
            BorderColor3 = Color3.fromRGB(0, 0, 0);
            BorderSizePixel = 0;
    ...
    do
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				if (enabled) then
					...
					local text = custom_text_enabled and '<font color="' .. text_color .. '">' .. features.get_text(custom_text) .. '</font>' or '<font color="' .. text_color .. '">AOC</font><font color="rgb(255, 255, 255)">OnTop</font>';
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				--// AOC classics
				local classics_enabled = flags["anti_aim_AOC_classics_enabled"];
				local classics_keybind = flags["anti_aim_AOC_classics_keybind"];
				local classics_types = flags["anti_aim_AOC_classics"];
				...
				if ((enabled or classics_enabled) and (keybind or classics_keybind) and (not (classics_keybind and classics_enabled and classics_types == "supercoolboi34 Destroyer") or (classics_keybind and classics_enabled and classics_types == "supercoolboi34 Destroyer"))) then
					...
					local types = {
						...
						["supercoolboi34 Destroyer"] = locals.should_AOC_destroy and hrp.CFrame + Vector3.new(0 / 0, 1, math.huge) or hrp.CFrame
					};
			...
			task.spawn(function()
				while task.wait(0.1) do
					locals.should_AOC_destroy = not locals.should_AOC_destroy;
				end;
			end);
...
do
	if (hookmetamethod) then
		...
		local __index; __index = hookmetamethod(game, "__index", LPH_NO_VIRTUALIZE(function(self, property)
			if (not checkcaller() and ((((flags["anti_aim_c_sync_enabled"] or (flags["anti_aim_AOC_classics_enabled"] and (flags["anti_aim_AOC_classics"] == "Destroy Cheaters" or flags["anti_aim_AOC_classics"] == "supercoolboi34 Destroyer"))) and (flags["anti_aim_c_sync_keybind"] or flags["anti_aim_AOC_classics_keybind"]))) and property == "CFrame" and self == local_player.Character.HumanoidRootPart)) then
...
do
	...
	do
		...
		local AOC_classics = ui.tabs["anti_aim"]:Section({Name = "AOC Classics", Side = "Left", Size = 150});
		...
		--// destroy cheaters
		do
			AOC_classics:Toggle({Name = "Enabled", Flag = "anti_aim_AOC_classics_enabled"});
			AOC_classics:Keybind({Flag = "anti_aim_AOC_classics_keybind", Name = "Keybind", Default = Enum.KeyCode.Y, Mode = "Toggle"});
			AOC_classics:List({Name = "Classics", Flag = "anti_aim_AOC_classics", Options = {"Destroy Cheaters", "supercoolboi34 Destroyer"}, Default = "Destroy Cheaters"});
		end;
	end;
	...
	do --// credits to finobe wtv im way too lazy
		...
		if not isfolder("AOC/configs") then 
		    makefolder("AOC/configs");
		end;

		local function update_config_list()
		    local list = {};
		
		    for idx, file in listfiles("AOC/configs") do
		        local file_name = file:gsub("AOC/configs\\", ""):gsub(".cfg", ""):gsub("AOC/configs/", "");
		...
		cfgs:Button({Name = "Create", Callback = function()
		    ...
		    if config_name == "" or isfile("AOC/configs/" .. config_name .. ".cfg") then
		        return;
		    end;
		    writefile("AOC/configs/" .. config_name .. ".cfg", Library:GetConfig());
		    update_config_list();
		end});
		...
		cfgs:Button({Name = "Save", Callback = function()
		    ...
		    if selected_config then
		        writefile("AOC/configs/" .. selected_config .. ".cfg", Library:GetConfig());
		    end;
		end});
		...
		cfgs:Button({Name = "Load", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        Library:LoadConfig(readfile("AOC/configs/" .. selected_config .. ".cfg"));
		    end;
		end});
		...
		cfgs:Button({Name = "Delete", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        delfile("AOC/configs/" .. selected_config .. ".cfg");
		    end;
		    update_config_list();
		end});
